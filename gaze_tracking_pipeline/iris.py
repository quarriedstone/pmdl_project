...
class Iris:
    ...
    def __init__(self, left, right, eye_frame, mask):
        ...
        self.r = self.shape[1] / 4 if SCALE == 1 else self.shape[1] / 5
    ...
    def _compute_rough_center(self, eye_frame):
        ...
        croped = croped.astype(float)
        croped = croped + (255 - self.mask)
        ...
        if SCALE == 2:
            croped[np.where(croped >= 200)] = 255
            croped[np.where(croped < 200)] = 0
        pupil = find(croped)
        return pupil
    ...
    def _iris_border_dilation(self, coords):
        ...
        dilated_polygon = cv2.dilate(polygon, kernel, iterations=8) / 255
